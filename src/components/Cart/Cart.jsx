...
const handleIncreaseQuantity = (itemId, quantity) => {
  // Dispatch the action creator to update the quantity
  ...
  const responsive = {
   ...
   return (  
      ...
      <div className="w-3/4 p-4 bg-white m-2">
        {/* header */}
        <h2 className="text-lg font-bold mb-4">Products ({cartItems.length})</h2>
        <hr></hr>
                {/* products */}
        <div>
        {cartItems.map((product, index) => (
          ...
          <div className="m-2  w-3/4 ">
             ...
             <p className="text-sm text-black-600 text-2xl " style={{fontSize:"20px"}}> EGY  {product.price}</p>
             <div className="flex items-center text-sm text-black-600 text-2xl justify-center">
                    <button onClick={() => handleDecreaseQuantity(product.product_id, product.quantity)}
                     className=" bg-orange-600 w-10 m-2 rounded-md font-bold text-2xl">-</button>
                    <p>{product.quantity}</p>
                    <button className=" bg-orange-600 w-10 m-2 rounded-md text-2xl"
                    onClick={() => handleIncreaseQuantity(product.product_id, product.quantity)}>+</button>
      ...
      {/* cart summary */}
      <div className="w-1/4 p-2 m-2 bg-white  font-bold  sticky top-20  absolute  shadow-lg rounded-lg"  style={{height:"fit-content"}}>
